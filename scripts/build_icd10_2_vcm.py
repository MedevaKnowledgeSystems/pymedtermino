...
for icd10 in ICD10.all_concepts():
  snomedcts = icd10 >> UMLS_ICD10 >> UMLS_SNOMEDCT >> SNOMEDCT
  ...
  for vcm in vcms:
    if vcm.concepts.find(VCM_CONCEPT[414]): # Tissular_structure
...
f = open("/tmp/icd10_2_vcm.txt", "w")
for icd10 in ICD10.all_concepts():
  if icd10.parents and (d[icd10] == d[icd10.parents[0]]): continue
  print((u" ".join([icd10.code, u"==", u" ".join(d[icd10]), u" #", icd10.term])).encode("utf8"), file = f)
f.close()

HERE        = os.path.dirname(sys.argv[0])
SQLITE_FILE = os.path.join(HERE, "..", "icd10_2_vcm.sqlite3")
db = create_db(SQLITE_FILE)
Txt_2_SQLMapping("/tmp/icd10_2_vcm.txt", db, "TEXT", "TEXT")
